...
export class UserManagementService {
  ...
  async addNewUser(
    createUser: CreateUserManagementDto,
    req: CustomRequest,
  ...
  Dưới đây là thông tin tài khoản của bạn:
        ...
        return { message: 'add user successful ', data: newUser };
      }
      ...
      if (req.user?.role === 'CompanyUser') {
        ...
        const newCompanyUser = await this.prismaService.user.create({
          data: {
            username,
            email,
            password: hashedPassword,
  ...
  Dưới đây là thông tin tài khoản của bạn:
        ...
        this.emailService.sendMail(email, `Welcome ${username}`, htmlContent);
        return { message: 'add user successful ', data: newCompanyUser };
      }
    } catch (error) {
  ...
  async resetPass(
    ...
    try {
      ...
      return { data: result, message: 'update password successful' };
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async editUser(
    id: number,
    infoUser: UpdateUserManagementDto,
  ): Promise<{ data: User; message: string }> {
    try {
      ...
      return { data: user, message: 'update password successful' };
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async softDelete(id: number): Promise<{ data: User; message: string }> {
    try {
      const result = await this.prismaService.user.update({
        data: {
          isDeleted: true,
      ...
      return { data: result, message: 'soft delete successful' };
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  async restoreUser(id: number): Promise<{ data: User; message: string }> {
    try {
      const result = await this.prismaService.user.update({
        data: {
          isDeleted: false,
      ...
      return { data: result, message: 'restore user successful' };
    } catch (error) {
      throw new HttpException(error.message, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
