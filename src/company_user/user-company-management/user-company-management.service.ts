...
@Injectable()
export class UserCompanyManagementService {
  constructor(
    private readonly prismaService: PrismaService,
    private readonly emailService: EmailService,
    @Inject('REDIS_CLIENT') private readonly redis: Redis,
  ) {}
  async addMemberForCompany(
    createUser: CreateUserCompanyManagementDto,
    req: CustomRequest,
  ) {
    console.log(createUser);
    const { email, username, role, dob, status, companyId } = createUser;
    await this.redis.set('email', email, 'EX', 10000);
    ...
    const who = await this.prismaService.user.findUnique({
    ...
    return { message: 'add user for company success' };
  }
  ...
  async resetPass(resetPass: ResetPassDto) {
    const emailUser = await this.redis.get('email');
  ...
  async editUser(
    id: number,
    infoUser: UpdateUserCompanyManagementDto,
    req: CustomRequest,
  ) {
    console.log(id);
    const who = await this.prismaService.user.findUnique({
    ...
    if (who.companyId !== companyId) {
      throw new HttpException(
        { message: 'Can edit this user' },
        HttpStatus.CONFLICT,
      );
    }
    await this.prismaService.user.update({
